...
def generate_enhanced_synthetic_datasets():
    ...
    farmer_aadhar = [f'{int(np.random.randint(1000,9999,1)[0]):04d}{int(np.random.randint(1000000000,9999999999,1)[0]):010d}' 
                     for _ in range(n_farmers)]
    farmer_phone = ['9' + f'{int(np.random.randint(1000000000,9999999999,1)[0]):010d}' for _ in range(n_farmers)]
    ...
    n_dealers = 500
    dealer_aadhar = [f'{int(np.random.randint(2000,2999,1)[0]):04d}{int(np.random.randint(1000000000,9999999999,1)[0]):010d}' 
    ...
    core_dealer_farmer = pd.DataFrame({
        ...
        'dealer_aadhar': '',  # Will populate from gov_dealers
        ...
        'claimed_fertiliser_qty_kg': np.random.uniform(500, 5000, core_size).round(0),  # Realistic fertilizer qty
    ...
    fraud_dealer_farmer = pd.DataFrame({
        ...
        'claimed_fertiliser_qty_kg': np.random.uniform(10000, 50000, fraud_size).round(0),  # Inflated fraud qty
    ...
    print("✅ ENHANCED DATASETS WITH AADHAR/PHONE GENERATED:")
    print(f"   📊 government_farmers.csv: {len(farmer_data):,} records (✅ Aadhar + Phone)")
    print(f"   🏪 government_dealers.csv: {len(dealer_data):,} records (✅ Aadhar)")
    print(f"   🤝 dealer_farmer_relationships.csv: {len(dealer_farmer_data):,} records (✅ Dealer Aadhar + Date + Fertiliser Qty)")
    
    print("\n🔍 NEW FRAUD DETECTION OPPORTUNITIES:")
    print("   • Aadhar mismatch across datasets")
    print("   • Phone number reuse across farmers")
    print("   • Inflated fertiliser quantities (>10,000kg flagged)")
    print("   • Fake Aadhar numbers")
